...
public class VillaController : ControllerBase
{
		...
		[FromQuery] int pageSize = 10)
	{
		...
		{
			var isDesceing = sortOrder?.ToLower() == "desc";

			villasQuery = sortBy.ToLower() switch
			{
				"name" => isDesceing ? villasQuery.OrderByDescending(u => u.Name)
				: villasQuery.OrderBy(u => u.Name),
				"rate" => isDesceing ? villasQuery.OrderByDescending(u => u.Rate)
				: villasQuery.OrderBy(u => u.Rate),
				"occupancy" => isDesceing ? villasQuery.OrderByDescending(u => u.Occupancy)
				: villasQuery.OrderBy(u => u.Occupancy),
				"sqft" => isDesceing ? villasQuery.OrderByDescending(u => u.Sqft)
				: villasQuery.OrderBy(u => u.Sqft),
				"id" => isDesceing ? villasQuery.OrderByDescending(u => u.Id)
